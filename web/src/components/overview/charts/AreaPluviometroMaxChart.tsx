...
export interface AreaPluviometroSeries {
  id: string;
  name: string;
  points: MonthlyMaxPoint[]; // 12 entries, months 1..12 in order
}
...
function renderLegend(series: AreaPluviometroSeries[]) {
  return (
...
/** One grouped bar per área — each pluviômetro in the área is its own series, so peak rain events are comparable side by side. */
export function AreaPluviometroMaxChart({ series }: { series: AreaPluviometroSeries[] }) {
  ...
  const chartData = MONTH_ABBR.map((month, i) => {
    ...
    for (const s of series) {
      ...
      row[s.id] = point?.hasData ? point.maxMm : null;
    }
    return row;
  });
    ...
    <ResponsiveContainer width="100%" height={240}>
      <BarChart data={chartData}>
        ...
        <Tooltip
          ...
          formatter={(value, name) => [value == null ? 'Sem leituras' : `${formatNumberBR(Number(value))} mm`, String(name)]}
        />
